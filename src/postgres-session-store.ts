...
/**
 * Postgres-backed SessionStore — mirrors chat session transcripts to external storage so a
 * session survives past the SDK's default local `~/.claude/projects/*.jsonl` files. Adapted from
 * the real reference adapter published in `anthropics/claude-agent-sdk-typescript`'s
 * `examples/session-stores/postgres/src/PostgresSessionStore.ts` (fetched and read in full before
 * writing this — that package isn't published to npm, per the SDK's own docs: copy and adapt, not
 * reinvent). One row per JSONL entry; ordering via BIGSERIAL.
 *
 * `listSessionSummaries` is deliberately not implemented here either, matching the reference —
 * `listSessions({sessionStore})` falls back to `listSessions()` + a per-session `load()` when it's
 * absent, which is an accepted v1 tradeoff (see the plan), not an oversight.
 */
export class PostgresSessionStore implements SessionStore {
  ...
  constructor(opts: PostgresSessionStoreOptions) {
    ...
    // Table name is interpolated directly into DDL/DML below since `pg` can't parameterize
    // identifiers — this check is load-bearing (SQL-injection guard via a caller-supplied table
    // name), not decorative.
  ...
  /** Creates the table + covering index if absent. Call once at startup, not per-query. */
  ...
  async append(key: SessionKey, entries: SessionStoreEntry[]): Promise<void> {
    ...
    // project_key/session_id/subpath repeat across rows; entry varies per row.
    ...
    await this.pool.query(`INSERT INTO ${this.table} (project_key, session_id, subpath, entry) VALUES ${rows.join(",")}`, params);
  }
  ...
  async listSessions(projectKey: string): Promise<Array<{ sessionId: string; mtime: number }>> {
    ...
    return rows.map((r) => ({ sessionId: r.session_id, mtime: r.mtime.getTime() }));
  }

  async delete(key: SessionKey): Promise<void> {
    if (key.subpath === undefined) {
      // Main-transcript delete cascades to all subpaths under (projectKey, sessionId).
      await this.pool.query(`DELETE FROM ${this.table} WHERE project_key = $1 AND session_id = $2`, [key.projectKey, key.sessionId]);
    } else {
      await this.pool.query(`DELETE FROM ${this.table} WHERE project_key = $1 AND session_id = $2 AND subpath = $3`, [
        key.projectKey,
        key.sessionId,
        key.subpath,
      ]);
